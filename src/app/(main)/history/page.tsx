      ...
      <div className="mb-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            {history.map((item) => (
              <div key={`${item.date}-${item.result_url}`} className="border-2 border-sky-400 rounded-lg p-4 dark:border-[#2AB7C6]">
      ...
      {showConfirm && (
        <div className="fixed inset-0 flex justify-center items-center z-50">
          <div className="bg-[#F4F9FF] dark:bg-[#161B22] p-6 rounded-lg border border-sky-400 dark:border-[#2AB7C6] shadow-lg max-w-sm w-full text-center">
            <p className="mb-4 text-black dark:text-white font-semibold p-4">Are you sure want to delete this history?</p>
            <div className="flex justify-center gap-4">
              <button onClick={handleDeleteConfirmed} className="px-4 py-2 bg-[#3674B5] dark:bg-[#161B22] text-white hover:bg-[#2a5f9e] border border-sky-600 dark:border-[#2AB7C6] rounded cursor-pointer">
                Yes
              </button>
              <button onClick={cancelDelete} className="px-4 py-2 bg-red-500 text-white hover:bg-red-700  rounded cursor-pointer">
                No
              </button>
            </div>
          </div>
